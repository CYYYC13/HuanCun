...
class SliceCtrl()(implicit p: Parameters) extends HuanCunModule {
  ...
  val io = IO(new Bundle() {
    ...
    val age_w = DecoupledIO(new AgeWrite())
  ...
  io.bin_w.bits.DLCounter.zipWithIndex.foreach {  // ignored
    case(m, i) =>
      m.D_L := 0.U
      m.L := 0.U
      m.L_sum := 0.U
  ...
  io.age_w.bits.repl_msg := 0.U.asTypeOf(io.age_w.bits.repl_msg)
